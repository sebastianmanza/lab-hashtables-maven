...
public class ProbedHashTable<K, V> implements HashTable<K, V> {
  ...
  void expand() {
    ...
    for (int i = 0; i < old.length; i++) {
      if (old[i] != null) {
        this.pairs[find(((Pair <K, V>)old[i]).key())] = old[i];
      }
    } // for
  ...
  int find(K key) {
    int index;
      for (index = Math.abs(key.hashCode()) % this.pairs.length; this.pairs[index] != null; index = (index + PROBE_OFFSET) % this.pairs.length) {
